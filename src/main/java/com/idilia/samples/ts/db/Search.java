...
@NamedQueries({
  @NamedQuery(
      name="Search.findUserRecent", 
      query="SELECT new com.idilia.samples.ts.db.SearchExpr(s.id, s.expression, s.senses) FROM Search s WHERE s.user = :user AND s.senses IS NOT NULL ORDER BY s.ranAt DESC")
})
public class Search {
  ...
  /** Keywords used to conclusively identify a document as matching the search expression */
  @Column(name="kws_pos")
  @Convert(converter=KeywordsConverter.class)
  private List<String> positiveKeywords;
  
  /** Keywords used to conclusively identify a document as not matching the search expression */
  @Column(name="kws_neg")
  @Convert(converter=KeywordsConverter.class)
   ...
   */
  public Search(User user, String expr) {
    this.user = user;
   ...
   * @param senses The results obtained by invoking "sensesAsObjects" on the tagging menu.
   */
