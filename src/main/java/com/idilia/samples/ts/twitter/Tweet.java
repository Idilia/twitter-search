...
public class Tweet implements Document {
  ...
  static abstract class Entity {
    int start;
    int end;
    Entity(int start, int end) { this.start = start; this.end = end; }
    abstract String format(String src);
  }
  
  static class HashTag extends Entity {
    String text;
    HashTag(String text, int start, int end) { super(start, end); this.text = text; }
    
    @Override
    String format(String src) {
      return "<a target=\"_blank\" class=\"tag\" href=\"https://twitter.com/hashtag/" + text + "?src=hash\")><span class=\"hash\">#</span><span class=\"hashtext\">" + text + "</span></a>";
    }
  }
  
  static class UserMention extends Entity {
    String name;
    UserMention(String name, int start, int end) { super(start, end); this.name = name; }

    @Override
    String format(String src) {
      return "<a target=\"_blank\" class=\"tag\" href=\"https://twitter.com/" + name + "\"><span class=\"at-sign\">@</span><span class=\"username\">" + name + "</span></a>";
    }
  }
  ...
  static class TCOLink extends Entity {
    ...
    TCOLink(String from, String to) { super(-1, -1); this.tco = from; this.expUrl = to; }
